...
export default function ImpactStory() {
  ...
  const [isCampusSubmitting, setIsCampusSubmitting] = useState(false)
  const [campusStatus, setCampusStatus] = useState<"idle" | "success" | "error">("idle")
  const [campusMessage, setCampusMessage] = useState("")

  const handleCampusSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    setCampusStatus("idle")
    setCampusMessage("")

    if (!campusName.trim() || !cityState.trim() || !contactName.trim() || !contactEmail.trim() || !contactPhone.trim()) {
      setCampusStatus("error")
      setCampusMessage("Please fill in all required fields before submitting.")
      return
    }

    setIsCampusSubmitting(true)
    try {
      const response = await fetch("/api/campus-invites", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          campusNote: campusNote.trim(),
        }),
      })

      if (!response.ok) {
        throw new Error("Failed to submit campus invite")
      }

      setCampusStatus("success")
      setCampusMessage("Thank you! Our outreach team will reach out shortly.")
      setCampusName("")
      setCityState("")
      setContactName("")
      setContactRole("")
      ...
      console.error("Campus invite failed", error)
      setCampusStatus("error")
      setCampusMessage("We couldn't send your request right now. Please try again or WhatsApp us.")
    } finally {
      setIsCampusSubmitting(false)
    }
  }

  return (
    <section id="impact" ref={ref} className="py-10 md:py-16 bg-background relative overflow-hidden">
  ...
  <p className="text-xs md:text-sm text-muted-foreground">or</p>

  <p className="text-xs md:text-sm text-muted-foreground">
    message us directly on WhatsApp after submitting the form.
  </p>
</div>
</div> 
          </div>
            ...
            <form onSubmit={handleCampusSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  value={campusNote}
                  onChange={(event) => setCampusNote(event.target.value)}
              ...
              <div className="mt-3 grid gap-3 md:grid-cols-[auto,1fr] items-center rounded-2xl bg-background/70 border border-border/60 p-3 md:p-4">
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-xl overflow-hidden bg-muted flex items-center justify-center mx-auto">
                  <img
                    src="QR code.png"
                    alt="Breezy WhatsApp QR for campus contact"
              ...
              {campusStatus === "success" && campusMessage && (
                <p className="text-xs md:text-sm text-emerald-600" aria-live="polite">{campusMessage}</p>
              )}
              {campusStatus === "error" && campusMessage && (
                <p className="text-xs md:text-sm text-red-600" aria-live="polite">{campusMessage}</p>
              )}
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={isCampusSubmitting}
                  className="inline-flex items-center justify-center rounded-full bg-primary px-6 py-2 text-sm font-semibold text-primary-foreground shadow-sm hover:bg-primary/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {isCampusSubmitting ? "Submitting..." : "Submit campus visit request"}
                </button>
              </div>
            </form>
          </div>
